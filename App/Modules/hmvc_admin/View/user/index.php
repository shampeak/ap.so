							...
							<form action=""  method="post">
								<table class="table table-hover">
									...
									<?php foreach($res as $value) { ?>
										<tr>
											<td><?=$value['userId']?></td>
											<td><?=$group[$value['groupId']]['groupname']?></td>
											<td><?=$value['login']?></td>
											...
											<td>
												<?php if ($value['active']) { ?>
													<a class="btn btn-primary btn-sm changestate"
													   ...
													   relid=<?= $value['userId']; ?>>有效</a>
													<?php
												} else {
													?>
													<a class="btn btn-warning btn-sm changestate"
													   data-target="#compose-modal" data-toggle="modal"
													   relid=<?= $value['userId']; ?>>无效</a>
													<?php
												}
												?>
												<a class="btn btn-primary btn-sm shamedit" relid=<?=$value['userId'];?>>编辑</a>
												<a class="btn btn-primary btn-sm shamview" relid=<?=$value['userId'];?>>查看</a>
												<a class="btn btn-primary btn-sm shamdelete" relid=<?=$value['userId'];?>>删除</a>
											</td>
										</tr>
									<?php }  ?>
									<tr>
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$('.shamedit').click(function(){
			...
			showAjaxModal('/admin/user/index/dialog/'+relid,'编辑用户')
		});

		$('.shamview').click(function(){
			var relid = $(this).attr("relid");
			showAjaxModal('/admin/user/index/ext/'+relid,'查看用户')
		});
		...
		$('.shamdelete').click(function(){
			if (confirm("确认要删除？")) {
				var relid = $(this).attr("relid");
				var res = $.ajax({
					url : '/admin/user/index/delete/'+relid,
					type: 'get',
					data: {},
					dataType: "json",
					async:false,
		...
		$('.changestate').click(function(){
			...
			var res = $.ajax({
				url : '/admin/user/index/states/'+relid,
				type: 'get',
				data: {},
				dataType: "json",
				async:false,
